...
export const AddFriendsDialog : FunctionComponent<Props> = ({ setOpenDialog, setFriendsData, friendsData }) => {
	...
	const requestUsersData = async () => {
		const bloquedFriends: {[key: string]: any} = await axios.get(`${process.env.REACT_APP_BACK_HOST}/users/${userId}/block`, { headers: authToken });
	...
	const handleSave = () => {
		...
		axios.post(`${process.env.REACT_APP_BACK_HOST}/users/${userId}/friends/by_name`, {
			"name": selectedUser[0]
